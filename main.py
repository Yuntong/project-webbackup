...
def main(arg):
    ...
    hp.add_argument("-d", "--jumptime", required = True,
                help = "时间间隔")
    hp.add_argument("-u", "--url", required = True,
                help = "备份网址")
    hp.add_argument("-o", "--output", required = True,
                help = "备份路径")
    args = vars(hp.parse_args())

    while 1:
        now = datetime.datetime.now().strftime('%Y%m%d%H%M')
        filepath = args["output"] + str(now)
        creatFolder(filepath)
        htmltext = scrapData(args["url"])
